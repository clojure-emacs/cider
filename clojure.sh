...
if [ ! -e $file ]; then
  ...
  output=$(2>&1 "$clojure" -Sforce -Srepro -J-XX:-OmitStackTraceInFastThrow -J-Dclojure.main.report=stderr -Sdeps '{:deps {mx.cider/tools.deps.enrich-classpath {:mvn/version "1.17.1"}}}' -M -m cider.enrich-classpath.clojure "$clojure" "$here" "true" "$@")
