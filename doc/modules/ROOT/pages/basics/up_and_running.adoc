...
In some situations it might be usefull to only start the nREPL server without
CIDER trying to automaticaly connect to it. This can help for complex setups
in which CIDER cannot reliably detect to which server:port to connect and
would therefore fail.
This assumes that the user executes a `cider-connect` command manualy afterwards
