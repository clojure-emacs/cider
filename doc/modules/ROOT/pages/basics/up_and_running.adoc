...
Using containers means that the JVM in which the nrepl server process is runnings, runs inside a container.
...
So Cider can hardly detect if it deals with remote files, so it is better to not rely on `cider-jack-in` and its remote support described above, but to 
start the nrepl server via command line from inside the container and `cider-connect` to it.
This requires to first get a shell inside the running container and  then start a nrepl server manualy or configure the container to start an nrepl automatically when container starts.
...
Docker has as well other ways to ensure the accessibility from containers ports on the host, not further detailed here.

=== Working with Containers running on remote hosts
In case we have the container runing on a remote machine, we need to to do the same setup as above and additionally use ssh to forward the already forwarded port again to our local machine. This can be done using a command such as "ssh -L  12345:localhost:12345 remote-server", asumming that 12345 was the port nrepl was exposed at by the container.

=== Working with `devcontainers` local or remote
`devcontainer` is a standard to describe container based development environments, comming with a command line tool.
...
There are several cli tools to manager container, as there are several container technologies. I will show an example using 
link:https://github.com/devcontainers/cli[devcontainer-cli], but their are others. (devpod,gitpod ...)
...
Then we can start a nrepl server inside the container on the remote host like below (executed from local)
...
----
"appPort": [
		"12345:12345", // will make container port 12345 available as 12345 on remote host (which will be further tunneled to 12345 on local machine)
	],
----

This results then in having port 12345 available localy and we can `cider-connect` to it. (using `localhost:12345`)
Editing of the files can then happen via TRAMP. As the files are "on the remote machine" and as well mounted inside the container on the remote machine, we have 2 TRAMP file syntaxes to edit hem.

Either: `/ssh:SERVER:/home/me/my-clj-code/...`
or: `/ssh:SERVER|docker:DOCKER_CONTAINER_ID:/workspaces/my-clj-code/...`
