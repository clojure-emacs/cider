...
this looks for `tools.deps`:
  ...
  $ clojure -Sdeps '{:deps {nrepl {:mvn/version "0.6.0"} cider/cider-nrepl {:mvn/version "0.42.0"}}}' -m nrepl.cmdline --middleware '["cider.nrepl/cider-middleware"]'
...
For this scenario, the `cider-start-nrepl-server` kbd:[C-c C-x s n]  command is offered, which optionally
...
[source,sh]
----
$ clj -Sdeps '{:deps {cider/cider-nrepl {:mvn/version "0.42.0"}}}' -m nrepl.cmdline --middleware "[cider.nrepl/cider-middleware]"
----
