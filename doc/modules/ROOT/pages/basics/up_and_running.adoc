...
In some situations it might be useful to only start the nREPL server without
CIDER trying to automatically connect to it. This can help for complex setups
in which CIDER cannot reliably detect to which server:port to connect and
would therefore fail.
This assumes that the user executes a `cider-connect` command manually afterwards
and specifies host:port .

For this there is the `cider-start-server` command, which optionally takes the same
parameters then `cider-jack-in`.
