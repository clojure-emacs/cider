...
In case we have the container runing on a remote machine, we need to do the same setup as above and additionally use ssh to forward the already-forwarded port again to our local machine. This can be done using a command such as "ssh -L 12345:localhost:12345 remote-server", assuming that 12345 was the nREPL port  exposed by the container.
...
There are several CLI tools to manage dev container, as there are several container technologies. I will show an example using 
...
Editing of the files can then happen via TRAMP. As the files are "on the remote machine" and as well mounted inside the container on the remote machine, we have two possible TRAMP file syntaxes to edit them.
