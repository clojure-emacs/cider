...
If you pause after typing kbd:[C-c C-x], a menu (`cider-start-menu`) pops up
...
* kbd:[-a] (`--aliases=`) - the Clojure CLI aliases to launch with
  (e.g. `:dev:test`)
* kbd:[-l] (`--cljs-repl=`) - the ClojureScript REPL type to use
* kbd:[-e] (`--edit-command`) - edit the final command in the minibuffer
  before it runs

Typing a full sequence like kbd:[C-c C-x j j] still fires immediately; the
menu only appears if you pause after the prefix.
...
xref:basics/remote.adoc[Remote and Container Connections]. Due
...
- `:dispatch-prefix-arg`, `:jack-in-type`, `:cljs-repl-type` -- used by
  `cider-jack-in-universal` to dispatch.  Tools without a
  `:dispatch-prefix-arg` are not selectable through that command.
...
The commands above assume the nREPL server runs on your own machine. For
servers on another host, inside a container, or reachable over a Unix domain
socket - including SSH tunneling and jacking in over TRAMP - see
