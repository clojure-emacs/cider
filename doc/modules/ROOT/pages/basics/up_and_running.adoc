...
This requires to first get a shell inside the running container and  start a nrepl server manualy or configure teh container to start an nrepl automatically when container starts.

In order to connect Emacs to teh nrepl, we need to make sure that the port of nrepl is reachable to our local Emacs, so that we can `cider-connect` to it.
