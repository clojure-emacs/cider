...
the build tool all run inside the container.  The tricky part is the
*connect-back* step - the container's name is not a host your Emacs can
reach, and CIDER's SSH tunneling doesn't apply to container methods.  CIDER
resolves this automatically when the container *publishes* the nREPL port:
it asks `docker port` (or `podman port`) which local port the server is
reachable on and connects there.  Since a jack-in's nREPL port is random by
default, this only helps if you fix the port and publish it; for anything
else, the approach described in the next section - start the server in the
container yourself, publish the port, and `cider-connect` to it - remains
the recommended one.
====

TIP: The same published-port resolution applies to `cider-connect`'s port
suggestions: from a `/docker:...` buffer, ports discovered inside the
container are offered as the local ports they're published on, and
unpublished (unreachable) ones are dropped.
