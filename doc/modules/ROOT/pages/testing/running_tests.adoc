...
dispatch to both the Clojure and the ClojureScript REPL, so you can exercise the
...
[#one-off-test-selectors]
=== One-off Test Selectors

For an ad-hoc selector filter you don't want to make permanent, use the test
menu that pops up when you pause after the kbd:[C-c C-t] prefix. It has two
arguments:

* kbd:[-i] (`--include=`) - only run tests tagged with these selectors
* kbd:[-x] (`--exclude=`) - skip tests tagged with these selectors

Set them once (space-separated, e.g. `:integration :flakey`) and they are
applied by the namespace (kbd:[n]), loaded (kbd:[l]) and project (kbd:[p]) run
commands invoked from the menu, without any prompting. The flags affect only
commands dispatched through the menu; the plain keybindings and the
