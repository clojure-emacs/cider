...
People typically install CIDER via `package.el`. While this gives you access to the
...
The main suite sets up its buffers with `clojure-mode`. To run the very same
specs on top of `clojure-ts-mode` instead (this is what CI does as an advisory
check), add `--under-clojure-ts-mode`:

 $ eldev[.bat] test --under-clojure-ts-mode
...
Eldev's `undercover` plugin is enabled, but stays inert unless you ask for a
report. To see which lines the unit tests reach:

 $ eldev test -u on,text
 # Or, as a SimpleCov-style JSON report with per-line hit counts:
 $ eldev test -u on,simplecov -U coverage.json
 # Or:
 $ make test-coverage

Don't pass `-p` here: coverage instrumentation needs the sources loaded as they
are, not byte-compiled.
