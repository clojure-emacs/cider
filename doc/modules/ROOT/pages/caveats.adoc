...
TIP: This page lists known limitations - things that don't work, or only work
partially, by design or because support is still in progress. If instead
something that's supposed to work is misbehaving, see
xref:troubleshooting.adoc[Troubleshooting]; for general questions see the
xref:faq.adoc[FAQ].
...
CIDER supports `clojure-ts-mode`, but that support is still maturing and it
currently leans on `clojure-mode` for some buffer analysis. See
