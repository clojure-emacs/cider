...
Sometimes there will be problems displaying completions when using a different value in
`completion-styles` that does not include `basic`, like when using
https://github.com/oantolin/orderless["orderless"]. That can be fixed by setting
`completion-category-defaults`, overriting the completion style just for the cider
complete at point function. The following snippet accomplish that:
