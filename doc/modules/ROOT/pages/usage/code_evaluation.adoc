...
Here are all three, on the same expression: kbd:[C-x C-e] with the cursor
after `+(* 2 3)+`, then `cider-eval-sexp-at-point` with the cursor on it, then
kbd:[C-M-x] from inside the enclosing form.

image::cider-form-selection.gif[Evaluating the preceding form, the form at the cursor, and the top-level form]
...
Here it is on one expression twice: kbd:[C-x C-e] flashes just the
`+(map :price items)+` call it sent, while kbd:[C-M-x] from inside the same
line flashes the whole `+defn+`.

image::cider-flash-evaluated-region.gif[The flash covering an inner call, then the whole enclosing defn]
