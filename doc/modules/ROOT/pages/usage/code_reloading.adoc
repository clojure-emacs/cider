...
TIP: The namespace menu that pops up when you pause after kbd:[C-c M-n]
exposes these modes as explicit, combinable arguments instead of prefix
arguments: kbd:[-a] (`--all`) reloads unconditionally, kbd:[-c] (`--clear`)
clears the tracker first, and kbd:[-i] (`--inhibit-fns`) skips the
`cider-ns-refresh-before-fn`/`cider-ns-refresh-after-fn` functions for this
run. Toggle the ones you want, then invoke the refresh with kbd:[r].
...
NOTE: After a successful `cider-ns-refresh`, `cider-ns-reload` or `cider-ns-reload-all`, CIDER refreshes the load-state indicators of the reloaded buffers (those with no unsaved edits), so they don't keep showing as stale. See xref:usage/code_evaluation.adoc#load-state-indicators[Evaluation and Load-State Indicators].
