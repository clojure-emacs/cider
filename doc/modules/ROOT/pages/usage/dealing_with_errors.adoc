...
[source,lisp]
----
(setq cider-auto-select-buffer '(test-report doc inspector cheatsheet log-frameworks)) ; keep selecting the rest
;; or, to never auto-select any of CIDER's popup buffers:
(setq cider-auto-select-buffer nil)
----

NOTE: `cider-auto-select-buffer` governs all of CIDER's popup buffers (error
buffer, test report, doc buffer, inspector, etc.).  It can be t, nil, or a
list of the buffers to select.  The former per-buffer option
`cider-auto-select-error-buffer` is obsolete as of CIDER 2.0, but still takes
...
Both targets also respond to kbd:[RET] (or kbd:[i]) with point on them, so you
don't have to reach for the mouse. This is defined and customizable in
`cider-stacktrace-exception-map` and `cider-stacktrace-ex-data-map`.

image::cider-stacktrace-inspect.gif[Inspecting an exception, and then its ex-data, from the error buffer]
