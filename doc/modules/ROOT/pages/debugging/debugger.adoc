...
image::cider-debugger.gif[A debugger session: stepping through a function with value overlays]
...
kbd:[C-M-x] (`cider-eval-defun`). CIDER can also
...
| kbd:[?]
| Open a menu (`cider-debug-menu`) listing all of the above, grouped and labelled
|===

TIP: There's no need to memorize this table - pressing kbd:[?] during a
debugging session pops up a https://github.com/magit/transient[transient]
menu with all the debugger commands, using the same keys.
