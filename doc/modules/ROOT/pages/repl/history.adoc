...
NOTE: The browser was renamed from `cider-repl-history` to `cider-history` in
CIDER 2.0, so its options no longer share a prefix with the unrelated REPL
input-history settings (`cider-repl-history-file`, `cider-repl-history-size`).
The old command and option names keep working as obsolete aliases.
...
== Repairing a Corrupted History

If a history entry with unbalanced parentheses sneaks into your saved
history, `cider-history` can fail to render with an "Unmatched
bracket or quote" error.  The command kbd:[M-x] `cider-repl-history-doctor`
walks the history, shows each entry whose parens don't balance in a side
buffer, and asks whether to delete it (kbd:[y]), keep it (kbd:[n]), or
stop reviewing (kbd:[q]).  When it finishes it rewrites
`cider-repl-history-file`, if one is configured.

== Mode

The history buffer has its own major mode,
`cider-history-mode`. This is derived from `clojure-mode`, so you
get fontification in the history buffer. This mode supports the
expected defcustom hook variable, `cider-history-hook`.
...
running `cider-history-quit` (see keyboard shortcuts).  Because
...
controlled by `cider-history-quit-action`, which can be assigned
...
and kills the `+*cider-history*+` buffer.
* `kill-and-delete-window` kills the `+*cider-history*+` buffer, and
deletes the window.
* `bury-buffer` simply buries the `+*cider-history*+` buffer, but keeps the
window.
...
By invoking `cider-history-occur` from the history buffer, you
...
When `cider-history-show-preview` is non-nil, CIDER displays an
...
it up with kbd:[M-x] `cider-history-clear-preview`.

By default, `cider-history-show-preview` is nil (disabled).
...
`cider-history-highlight-inserted-item`, which can be set to the
following values:
...
`cider-history-highlight-inserted-item`.

When `cider-history-highlight-inserted-item` is non-nil, you
can customize the face used for the inserted text with the variable
`cider-history-inserted-item-face`.
...
* `cider-history-display-duplicates` - when set to `nil`, will not display any
duplicate entries in the history buffer.  Default is `t`.
* `cider-history-display-duplicate-highest` - when not displaying duplicates,
...
* `cider-history-display-style` - the history entries will often be more than
...
* `cider-history-separator` - when `cider-history-display-style` is `separated`,
...
* `cider-history-maximum-display-length` - when nil (the default), all history
...
* `cider-history-recenter` - when non-nil, always keep the current entry at the
top of the history window.  Default is nil.
* `cider-history-resize-window` - whether to resize the history window to fit
...
* `cider-history-highlight-current-entry` - if non-nil, highlight the currently
selected entry in the history buffer.  Default is nil.
* `cider-history-current-entry-face` - specifies the face for the history-entry
highlight.
* `cider-history-text-properties` - when set to `t`, maintains Emacs text
