...
Note that CIDER stores the history in a file called `.cider-history`
located at the root of your project if the `cider-repl-history-file`
is not set. It writes to it when you kill the REPL buffer, which
includes invoking `cider-quit`, or when you quit Emacs.
