...
* You can pop up a quick REPL reference card with kbd:[C-c C-h] (`cider-repl-help`)

On top of this the REPL is extremely configurable and you can tweak almost every aspect of it.

TIP: New REPL buffers open with a compact banner and a one-line hint pointing at
kbd:[C-c C-h] (also the `,refcard` REPL shortcut), which summons a `cider-repl-help`
reference covering the common REPL interactions, loading code, switching
