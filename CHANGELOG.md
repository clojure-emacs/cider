...
- [#4156](https://github.com/clojure-emacs/cider/pull/4156), [#4162](https://github.com/clojure-emacs/cider/pull/4162): Add `cider-form-targeting`: when set to `smart`, the `last-sexp` commands (`cider-eval-last-sexp`, `cider-macroexpand-1`, `cider-inspect-last-sexp`, etc.) resolve their target form from the cursor position (on a delimiter, inside a form) instead of requiring point to sit right after it; the default `preceding` keeps the classic behavior. The eval menu (`C-c C-v`) shows the active mode and `T` toggles it for the session.
