...
- [#4153](https://github.com/clojure-emacs/cider/pull/4153): Report the jack-in command in `cider-doctor`, right below its cider-nrepl version check, and add `cider-repl-describe-startup` and `cider-repl-help` to the REPL menu.
...
- [#4188](https://github.com/clojure-emacs/cider/pull/4188): Fix the `cider-connect` port suggestions taken from open REPL buffers coming out with a quoted port (`proj:"1234"`) and being silently discarded.
...
- [#4157](https://github.com/clojure-emacs/cider/pull/4157): Show the full Clojure version in `cider-doctor` (e.g. `1.13.0-alpha6`), instead of truncating the qualifier.
...
- [#4151](https://github.com/clojure-emacs/cider/pull/4151): Fix the inspector's uninteresting-values regexp (used by `cider-inspector-skip-uninteresting`) treating commas as digits, which was a `[+-.]` character range covering `,`.
...
- [#4147](https://github.com/clojure-emacs/cider/pull/4147): Fix macrostep expansion and eval-to-comment signaling `wrong-type-argument integerp` in `clojure-ts-mode` buffers on Emacs 30.1 (marker positions were passed to `indent-region`); the macrostep popup now honors `cider-preferred-clojure-mode` too.
