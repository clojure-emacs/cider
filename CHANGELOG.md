...
- Experimental support for connecting to Clojure's prepl (`clojure.core.server/io-prepl`). New commands: `cider-jack-in-prepl`, `cider-connect-prepl`, `cider-prepl-eval-string`/`region`/`last-sexp`/`defun-at-point`, `cider-prepl-load-file`, `cider-prepl-set-ns`, `cider-prepl-doc`, `cider-prepl-show-tap-buffer`, `cider-prepl-clear-output`, `cider-prepl-quit`, `cider-prepl-restart`. A slim REPL UI (`cider-prepl-mode`, comint-based) tracks the prepl's current namespace in the prompt and routes `:tap` values to a per-connection buffer. Eval-form fallbacks reshape `info`, `eldoc`, `complete`, `apropos`, `ns-vars`, `ns-list`, `source`, `macroexpand`, and `classpath` op calls into nREPL-shaped responses. See `repl/prepl.adoc` for the full picture; expect rough edges -- prototype quality, not yet wired into `cider-connect` proper.
- `cider-jack-in-tools` gains `:backend` (default `nrepl`) and `:server-args-fn` slots so the registry can describe non-nREPL tools.  `cider-jack-in-universal` dispatches on `:backend`, so prefix-arg `M-6` jacks in via prepl on deps.edn projects (registered as `clojure-cli-prepl`).  Existing third-party `cider-register-jack-in-tool` callers are unaffected -- the default `nrepl` backend keeps the historical behavior.
- The standard `cider-eval-last-sexp`, `cider-eval-region`, `cider-eval-defun-at-point`, `cider-load-file`, and `cider-doc` commands now dispatch on the active backend (`cider-current-backend`).  When a Clojure source buffer is connected only to a prepl session, the usual keybindings (`C-x C-e`, `C-c C-k`, `C-c C-d C-d`, ...) Just Work -- no need to reach for the `cider-prepl-*` siblings.  When both nREPL and prepl are linked, nREPL takes priority.
- `cider-jack-in-prepl` honors TRAMP -- when `default-directory` is remote, the JVM is spawned on the remote host via `start-file-process`.  New customs `cider-jack-in-prepl-bind-address` (for the JVM's `:address`) and `cider-jack-in-prepl-host` (for the connect-side host) let the user choose between binding on `0.0.0.0` for direct connect vs. a pre-established SSH tunnel.  Free-port discovery is local-only, so `cider-jack-in-prepl-port` must be set explicitly when remote.
