  ...
  (cond ((string-match "^file:\\(.+\\)" resource)
	...
	((string-match 
	  (concat "^" tramp-default-method 
		  "\\(.+\\)@\\(.+\\):\\(jar\\|zip\\):file:\\(.+\\)!/\\(.+\\)") 
	  resource)
	 (let* ((jar (match-string 4 resource))
		(path (match-string 5 resource))
		(user (match-string 1 resource))
		(host (match-string 2 resource))
		(buffer-already-open (get-buffer (file-name-nondirectory jar))))
	   (find-file (concat "/" tramp-default-method ":" user "@" host ":" jar))
	 ))
...
;; klang - suggestion
;; taking the username and ip-address of the location into account
